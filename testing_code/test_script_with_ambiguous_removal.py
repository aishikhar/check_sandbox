...
def get_iou(true, pred):
        """
        Adapted from get_pq_fast/stats_utils/ : Hover-Net: Simon Graham .et.al. 
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ambi_path', help='path to ambigious region masks')
    ...
    if args.ambi_path:
        ambi_path = args.ambi_path
    if args.gpu:
        os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
        n_gpus = len(args.gpu.split(','))

    submission = Submission()
    submission.run(output_dir=output_dir, data_dir=data_dir, model_path = model_path, ambi_path = ambi_path, img_ext = img_ext)    
